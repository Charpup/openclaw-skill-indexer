...
server.registerTool("search_skills", {
    title: "Search Skills",
    description: "Search OpenClaw skills by keyword, name, or description. Returns matching skills with pagination metadata. Does NOT modify any data.",
    inputSchema: {
    ...
    try {
        ...
        const allResults = index.skills.filter((skill) => skill.id.toLowerCase().includes(q) ||
...
server.registerTool("get_skill_info", {
    ...
    description: "Get detailed information about a specific skill by its ID (directory name). Returns full metadata including triggers, version, and configuration.",
    inputSchema: {
        skill_id: z
            .string()
            .min(1)
            .describe("Skill ID (directory name, e.g. 'triadev', 'skill-indexer')"),
    },
    annotations: {
        readOnlyHint: true,
        destructiveHint: false,
...
server.registerTool("list_skills", {
    title: "List Skills",
    description: "List all indexed OpenClaw skills with optional type filtering and pagination. Returns skills array with total count and pagination metadata.",
    inputSchema: {
        type: z
            .enum(["system", "workspace", "all"])
            .default("all")
            .describe("Filter by skill type: 'system' (from ~/.openclaw/skills), 'workspace', or 'all'"),
        limit: z
            .number()
            .int()
            .min(1)
...
server.registerTool("find_by_trigger", {
    title: "Find by Trigger",
    description: "Find skills registered for a specific trigger keyword (exact match). Returns all skills that activate on the given trigger phrase.",
    inputSchema: {
        trigger: z
            .string()
            .min(1)
            .describe("Exact trigger keyword to look up (e.g. 'triadev', 'search docs')"),
    },
    annotations: {
        readOnlyHint: true,
        destructiveHint: false,
    ...
    try {
        ...
        return {
            content: [
                {
                    type: "text",
                    text: JSON.stringify({ trigger, count: results.length, skills: results }, null, 2),
                },
            ],
        };
    }
...
server.registerTool("refresh_index", {
    ...
    description: "Get current skill index statistics: version, generation timestamp, and skill counts. Does NOT rebuild the index — to rebuild, run: skill-indexer index",
    inputSchema: {},
    annotations: {
        readOnlyHint: true,
        destructiveHint: false,
