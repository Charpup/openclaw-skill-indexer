...
server.registerTool("search_skills", {
    ...
    const results = index.skills.filter((skill) => skill.id.toLowerCase().includes(q) ||
        skill.name.toLowerCase().includes(q) ||
        skill.description.toLowerCase().includes(q)).slice(0, limit);
    return {
        content: [{
                type: "text",
...
server.registerTool("find_by_trigger", {
    ...
    const results = ids.map((id) => index.skills.find((s) => s.id === id)).filter(Boolean);
    return {
        content: [{
                type: "text",
                text: JSON.stringify(results, null, 2)
